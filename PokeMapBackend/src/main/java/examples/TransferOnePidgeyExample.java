...
public class TransferOnePidgeyExample {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//PokemonGo go = new PokemonGo(new PtcCredentialProvider(http, "aariasta@gmail.com", "arce5720273"), http);
			//CredentialProvider cre = new PtcCredentialProvider(http, "aariasta@gmail.com", "");

			/*GoogleCredentialProvider.OnGoogleLoginOAuthCompleteListener onGoogleLoginOAuthCompleteListener = new GoogleCredentialProvider.OnGoogleLoginOAuthCompleteListener() {
				@Override
				...
				public void onTokenIdReceived(GoogleAuthTokenJson googleAuthTokenJson) {
					System.out.println(googleAuthTokenJson.getIdToken());
				}
			};*/

			//GoogleCredentialProvider auth = new GoogleCredentialProvider(http, onGoogleLoginOAuthCompleteListener); // currently uses oauth flow so no user or pass needed

			//CredentialProvider auth = new GoogleCredentialProvider() // currently uses oauth flow so no user or pass needed
